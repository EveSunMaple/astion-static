import type { Config } from "./utils/interfaces";
import * as fs from "node:fs";
import * as path from "node:path";
import yaml from "js-yaml";

// 配置文件路径
const configPath = path.resolve("astion.config.yaml");
// 读取并解析 YAML 文件
...
// 用户侧边栏信息
export const USER_SIDEBAR_TITLE = config.user.sidebar.title;
export const USER_SIDEBAR_DESCRIPTION = config.user.sidebar.description;
